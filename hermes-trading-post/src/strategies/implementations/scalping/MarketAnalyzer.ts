...
/**
 * Handles market analysis for scalping strategies
...
export class MarketAnalyzer {
  ...
  updateRecentHigh(candles: CandleData[], currentPrice: number): void {
    ...
    if (this.state.currentLevel === 0) {
      // Don't wait for lookback period - trade immediately!
      if (this.state.recentHigh === 0 && candles.length > 0) {
        ...
        console.log(`[MarketAnalyzer:${this.variant}] Initialized to first candle high ${this.state.recentHigh.toFixed(2)} - ready to trade immediately!`);
      }
      ...
      // Use a rolling window only after we have enough candles
      if (candles.length >= this.config.lookbackPeriod) {
        ...
        if (lookbackHigh > this.state.recentHigh || this.state.recentHigh === 0) {
          ...
          console.log(`[MarketAnalyzer:${this.variant}] Updated to ${lookback}-candle high: ${this.state.recentHigh.toFixed(2)}`);
        }
      } else {
        // Not enough candles for lookback - use absolute high
        this.state.recentHigh = absoluteHigh;
        console.log(`[MarketAnalyzer:${this.variant}] Using absolute high (only ${candles.length} candles): ${this.state.recentHigh.toFixed(2)}`);
      }
      
      // Always update if current price is a new high
      if (currentPrice > this.state.recentHigh) {
        this.state.recentHigh = currentPrice;
        console.log(`[MarketAnalyzer:${this.variant}] New absolute high: ${this.state.recentHigh.toFixed(2)}`);
      }
    } else {
      // When in a position, only update if we see a new high
      if (currentPrice > this.state.recentHigh) {
        this.state.recentHigh = currentPrice;
        console.log(`[MarketAnalyzer:${this.variant}] New high while in position: ${this.state.recentHigh.toFixed(2)}`);
      }
    }
  }
  ...
  logMarketStatus(candles: CandleData[], currentPrice: number, availableBalance: number): void {
    ...
    if (candles.length <= 50 || dropFromHigh >= 3 || candles.length % 250 === 0) {
      console.log(`[MarketAnalyzer:${this.variant}] Market status:`, {
